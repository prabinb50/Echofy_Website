    ...
    return (
        <div className='relative z-0'>
            ...
            <div className='absolute top-0 left-0 w-full h-full flex items-center justify-center '>
                <div className='grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 lg:gap-12 xl:gap-25 w-11/12 max-w-7xl'>
                    {/* loop through stats */}
                    {stats.map((stat, index) => (
                        <div key={index} className='space-y-2 md:space-y-4 lg:space-y-6 text-center py-5 md:py-0'>
                            ...
                            {/* count */}
                            <span className='text-white text-2xl md:text-3xl lg:text-4xl font-bold block'>{stat.count}</span>

                            {/* label */}
                            <p className='text-white text-base md:text-lg lg:text-xl'>{stat.label}</p>
                        </div>
                    ))}
                </div>
            </div>
